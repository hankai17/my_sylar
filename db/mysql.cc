...
namespace sylar {
    ...
    MySQLRes::ptr MySQLManager::query(const std::string &name, const char *format, const std::string &sql) {
        auto conn = get(name);
        if (!conn) {
            SYLAR_LOG_ERROR(g_logger) << "MySQLManager::query get(" << name
                                      << ") failed sql: " << sql;
