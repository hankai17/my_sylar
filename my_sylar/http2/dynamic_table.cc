...
// kv均在静态表中 则首位传1 剩余7位传索引号 eg: method: GET在索引中为2 即用1000 0010即0x82表示
// k在静态表中 v不在静态表中 且将v新增至动态表中 则首两位传01 eg: if-none-match: "5cb816f5-19d8" 我们知道if-none-match
//   在静态表中的序号为41 即用01101001表示 再看其value 长度为15 再采用huffman编码
// kv均不在静态表中 且将kv新增至动态表中 则首两位传01 
// k在静态表中 v不在静态表中 且不需将v新增至动态表中 则前四位传0000
